
//import jdk.nashorn.internal.runtime.ECMAException;

import java.awt.Point;
import java.io.BufferedReader;
import java.io.FileReader;
import java.util.ArrayList;
import java.util.List;

public class World {
    public static String[][] _world; //two dimensional string array

    static Point Starting_Position = new Point(0,0);

    // This method loads the tiles from the map.txt file and populates the _world array

    ///Users/ojakanbi/Desktop/TextAdv/src
        public void load_tiles() {
            // Parses a file that describes the world space into the _world object
            List<String> rows = new ArrayList<>();

            try{
                BufferedReader f =new BufferedReader(new FileReader(("/Users/ojakanbi/Desktop/TextAdv/src/map.txt")));
                String row;
                while  ((row = f.readLine()) != null ) {
                    rows.add(row);

                }
                f.close();
                int x_max = rows.get(0).split("\t").length;
                _world = new String[rows.size()][x_max];
                String[] cols;
                String tile_name;
                for (int y =0; y < rows.size(); y++){
                    cols = rows.get(y).split("\t");
                    for (int x = 0;x < x_max; x++) {
                        tile_name = cols[x];
                        if (tile_name.equals("StartingRoom")){
                            Starting_Position.x = y;
                            Starting_Position.y = x;
                        }
                        _world[y][x] =tile_name.equals(" ") ? null :tile_name;
                    }

                }


            } // closing try block

            catch (Exception e) {
                // System.err.format( "Exception occurred trying to read in the file name)
                e.printStackTrace();
            }
        } // closing block for load_tiles Method
    public static MapTile tile_exists(int x, int y){
        MapTile mt = null;

        //Todo: verify the x and y exist in the world
        if ((x >= 0 && x < _world.length) && (y >= 0 && y < _world[0].length)
                && _world[x][y] != null){
            switch (_world[x][y]){
                case "StartingRoom":
                    mt = new StartingRoom(x, y);
                    mt = checkRoomExist(mt);
                    break;
                case "FindPistolRoom":
                    mt = new FindPistolRoom(x, y);
                    mt = checkRoomExist(mt);
                    break;
                case "GiantSquidRoom":
                    mt = new GiantSquidRoom(x, y, new GiantSquid());
                    mt = checkRoomExist(mt);
                    break;
                case "TreasureRoom":
                    mt = new TreasureRoom(x, y, new Gold(15));
                    mt = checkRoomExist(mt);
                    break;
                case "EmptyGrottoPath":
                    mt = new EmptyGrottoPath(x, y);
                    break;
                case "LeaveGrottoRoom":
                    mt = new LeaveGrottoRoom(x, y);
                    mt = checkRoomExist(mt);
                    break;

            }
        }
        return mt;

    }

    private static MapTile checkRoomExist(MapTile mt) {
        // Read the map.txt file to verify if the given MapTile object exists
        try {
            BufferedReader f = new BufferedReader(new FileReader("/Users/ojakanbi/Desktop/TextAdv/src/map.txt"));
            String line;
            while ((line = f.readLine()) != null) {
               if (line.contains(mt.getClass().getSimpleName())) {
                    f.close();
                    return mt;
                }
            }
            f.close();
        } catch (Exception e) {
            e.printStackTrace();
        }

        // The MapTile object does not exist in the map.txt file
        return null;
    }


}





